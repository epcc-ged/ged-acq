...
<p class="biblio">
	<span>Filtre par territoire :
		<select class="biblio" id="filtre_territoire">
			...
			<option value="A">A : Connaissances et savoirs</option>
			<option value="B">B : Histoire</option>
			<option value="C">C : Textes, sens, création</option>
			<option value="D">D : Espaces, populations, sociétés</option>
			<option value="E">E : Afrique</option>
			<option value="F">F : Amériques</option>
			<option value="G">G : Asie</option>
			<option value="H">H : Eurasie</option>
		</select>
	</span>
...
foreach($xml_result->QueryResult->ResultXml->rowset->Row as $row){
  $segment = give_segment($row->Column7);
  echo '<div class="segment '.$segment.'">';
  ...
  echo '<p><a class="ged" href="' . $row->Column14 . '">Voir la notice dans le catalogue</a></p></li>';
  echo '</div>';
}
echo '</ul>';
//var_dump($response);
